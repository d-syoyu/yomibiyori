...
def get_season_info(target_date: date) -> str:
    ...
    if month == 1:
        if day == 1:
            return (
                "元旦（1月1日）。新しい年の始まり、初日の出、初詣の人混み、おせち料理、お雑煮、"
                "お年玉、年賀状、家族団らん、新年の抱負、書き初め、「あけましておめでとう」の挨拶、"
                "振袖・着物、初笑い、初夢、一年の計は元旦にあり"
            )
        elif day <= 3:
            return (
                "お正月（1月三が日）。初詣、おせち料理、お年玉、年賀状、箱根駅伝、"
                "こたつでみかん、正月番組、親戚の集まり、初売りセール、ゆっくりした時間"
            )
...
class OpenAIThemeClient(ThemeAIClient):
    ...
    CATEGORY_PROMPTS = {
        ...
        "ユーモア": (
            ...
            "4. 【ブラックユーモア】人生の不条理、存在の虚しさ、世代間ギャップ、"
